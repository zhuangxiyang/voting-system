...
import com.vote.common.util.SpringUtils;
...
public class WriteInterceptor extends AbstractSqlParserHandler implements Interceptor {
    ...
    public Object intercept(Invocation invocation) {
        ...
        if (!SpringUtils.getApplicationContext().getEnvironment().getProperty("vote.database.isNotWrite", Boolean.class, false)) {
            return invocation.proceed();
        }
...
//        if (StrUtil.containsAnyIgnoreCase(mappedStatement.getId(), "UserToken", "sms", "MsgsCenterInfo", "resetPassErrorNum", "updateLastLoginTime", "OptLog", "LoginLog", "File", "xxl")) {
        ...
        // userId=1 的超级管理员 放行
        String userCode = BaseUserUtil.getCurrentUser().getUserCode().toString();
//        String tenant = BaseContextHandler.getTenant();
        log.info("mapperid={}, userCode={}", mappedStatement.getId(), userCode);


        //演示用的超级管理员 能查 和 增
//        if (userCode.equals("2") && (DELETE.equals(mappedStatement.getSqlCommandType()))) {
...
//            throw new BizException(-1, "演示环境禁止修改、删除重要数据！请在 vote-admin-ui 使用其他租户登录后测试 全部功能");
//        }
